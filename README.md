<img width="2030" height="1080" alt="combat cc" src="https://github.com/nikoladhima/combat.cc/blob/main/icons/combat.cc.png"/>

## COMBAT.CC [UNIVERSAL]

> [!IMPORTANT]
> **Join my [discord server](https://discord.gg/DwRT2nH93D) if you find any bugs or have issues with the scripts,**
> **If you are on a Mobile Device then its recommended to use a keyboard script or an actual keyboard.**

> [!WARNING]
> **This script only works in games that use the "Character" property under every Player and the Default BodyParts, this means that this script will NOT work in some games like Phantom Forces, Bad Business & Frontlines but you shouldn't worry because there are only a few games like this.**

## ⚙️ Features:
- Aimbot 🎮
- TriggerBot 🎮
- Client Manipulation 🌐
- AND MANY MORE FEATURES, CHECK MY WEBSITE: https://combat-cc.vercel.app

## 🔌Shortened Script
```lua
--[=[
 ██████╗ ██████╗ ███╗   ███╗██████╗  █████╗ ████████╗    ██████╗ ██████╗
██╔════╝██╔═══██╗████╗ ████║██╔══██╗██╔══██╗╚══██╔══╝   ██╔════╝██╔════╝
██║     ██║   ██║██╔████╔██║██████╔╝███████║   ██║      ██║     ██║     
██║     ██║   ██║██║╚██╔╝██║██╔══██╗██╔══██║   ██║      ██║     ██║     
╚██████╗╚██████╔╝██║ ╚═╝ ██║██████╔╝██║  ██║   ██║   ██╗╚██████╗╚██████╗
 ╚═════╝ ╚═════╝ ╚═╝     ╚═╝╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚═╝ ╚═════╝ ╚═════╝
    -- // Made by nikoleto scripts - github.com/nikoladhima \\ --
]=]
loadstring(game:HttpGet("https://raw.githubusercontent.com/nikoladhima/combat.cc/main/combat.cc.lua"))()
```
<br/>
